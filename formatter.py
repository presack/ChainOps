...
def format_cli_report(result: dict[str, Any]) -> str:
    ...
    if "error" in blockstream:
        ...
        lines.append(f"Balance: {blockstream.get('balance_btc', 0):.8f} BTC ({blockstream.get('balance_sats', 0)} sats)")
    ...
    if "error" in price:
        ...
        if usd is not None and "error" not in blockstream:
            value_usd = blockstream.get("balance_btc", 0) * usd
